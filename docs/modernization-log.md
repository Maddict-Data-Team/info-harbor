...
and the gates — not the classifier — decide which instance a run may
touch. It is never accepted as a source, because Automation writes those
tables. `UNRESOLVED_DEPENDENCY` is kept distinct so `<code>_streach`
(plan lines 151-153: read by the OOH query, no producer in this
repository) stays visible as the open question it is. Patterns are
exact: POI dataset/table pairs are derived from `COUNTRY_POI_TABLES`, so
`POI_DB_KSA.All_POIs_UAE` cannot match; output names are enumerated
against the real `queries = ` vocabulary and the INTEGER campaign-code
format, so an unexpected table surfaces as `None` rather than being
absorbed.
...
`tests/conftest.py`, `pytest.ini`, `docs/code-audit.md`). `origin/dev`
carries none of them, so these tests could not be collected there.
